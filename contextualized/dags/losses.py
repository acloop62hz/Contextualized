...
def dag_loss_dagma_indiv(w, s=1，device = 'cuda'):
    w = w.to(device)
    M = s * torch.eye(w.shape[-1],device=device) - w * w
    return w.shape[-1] * np.log(torch.tensor(s, device=device)) - torch.slogdet(M)[1]
